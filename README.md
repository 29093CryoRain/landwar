...
用 **SDL2 + entt（ECS）+ Dear ImGui** 重写的领土战争实时模拟游戏。多个可配置 AI 势力在 105×95 格网
...
> **射弹系统（P9）**：新增**手枪兵**（每 120 tick 向正前方发 1 颗恒速 0.5 U/tick 子弹，成本 80）与
> **霰弹兵**（每 180 tick 向正前方 40° 扇形均布+轻抖动发 3 颗随机速度 1/3±1/15（基准值
> 0.8~1.2 倍，2026-08 细节改进：原 ±1/6 方差过大）U/tick 子弹，
...
> **菜单（P1/P6）**：窗口模式默认先进入主菜单；势力区显示本局已选列表，最下方「添加势力」打开配置势力库，选择势力和产兵 AI 后追加到列表尾部。点「选择地图…」

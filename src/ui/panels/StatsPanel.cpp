...
void drawFactionUnitTable(const Simulation& sim, int fid) {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "势力 × 兵种（%s）", factionShortName(fid));
...
void drawFactionSummary(const Simulation& sim) {
    ...
    if (ImGui::BeginTable("##stats-fs", 4, ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_Borders)) {
        ...
        for (int fid = 1; fid <= kPlayerFactionCount; ++fid) {
            ...
            ImGui::TextUnformatted(factionShortName(fid));
            ImGui::PopStyleColor();
            ImGui::TableSetColumnIndex(1);
...
void StatsPanel::draw(PanelCtx& ctx) {
    ...
    if (ImGui::BeginCombo("势力", factionShortName(selFaction))) {
        for (int fid = 1; fid <= kPlayerFactionCount; ++fid) {
            if (ImGui::Selectable(factionShortName(fid), fid == selFaction)) selFaction = fid;
        }
        ImGui::EndCombo();
    }

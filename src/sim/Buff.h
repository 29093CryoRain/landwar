...
struct Buff {
    ...
    double magnitude = 1.0;  // 乘数（LaserExtraBeams / ProjectileCountExtra 为条数）
...
//   - 加算型（增幅）：result = 1 + Σ(m−1)——速度/经济/半径/寿命/长度/宽度/地雷超时/射速/科技点；
...
struct FactionMods {
    ...
    double bombRadiusMult = 1.0;     // 爆炸半径乘数
...
// 数值文案（科研弹窗/科技面板用）：加算/乘算 "+X%"/"-X%"（按 (m-1)×100 取整），条数 "+N"。

...
> 当前进度：1.2 已完成。完成 1.2 后清理了部分无用代码，并将基线从完整快照 hash 改为上述语义基线；
...
- **现状**：`MapRenderer::drawTiled` 山地贴图尺寸 = `cellPx`（`MapRenderer.cpp:171`），
  中心 = `cellCenter`（质心）。
- **做法**：尺寸 `size ∝ √(cellArea)`（缩放比例平方正比面积），中心 = 4.3 的 `cellIncenter`；
  方形路径（`drawSquare:80-90`）面积恒等、内切圆=中心，行为不变。
...
### 5.0 非方/六/三密铺临时基线（性能优化前置，原文 13 配套）

- **现状**：`tests/test_baseline.cpp` 只锁方/六/三的确定性语义基线（`runRandomBaseline`，
  随机图 120×120、种子 42、陆地 0.5、强制边缘海、山 0.1、城 0.015、2500 tick）；
  其余 14 种 arch/laves 仅有"同 seed 同语义 hash"快测（`SmallTiledAllTilingsSameSeedSameHash`），
  没有固定 hash 期望值。
- **做法**：对全部 14 种 arch/laves 密铺各跑一条**临时基线**，参数与方/六/三完全一致
  （120×120 随机图、种子 42、2500 tick），记录各密铺的**语义基线 hash**（临时表/临时测试，不进
  `test_baseline.cpp` 永久期望）。这一步在优化**前**完成，作为优化前后语义行为不变的对照基准。
- **用途**：5.1-5.4 优化完成后逐密铺比对语义 hash——必须逐位一致；若某密铺 hash 漂移，
  说明优化改了 worldToCell 扫描序或语义结果，须修正或回退。若只发现经济、科技、特效、RNG
  或其它未纳入字段变化，应另做完整快照回归，不要仅凭语义 hash 判断。
- **涉及**：`tests/test_baseline.cpp`（新增临时用例，参数同 `runRandomBaseline`）、
  `tools/gen_table_domain_params.py`（不动）。
- **风险**：低（纯测试新增；运行时长增加 14×2500 tick，可标 `DISABLED_` 或用小参数快测）。
- **验收**：优化前后各密铺临时语义基线逐位一致；必要时补充快照回归；`ctest` 全绿。

### 5.1 直线判定优先的解析优化（原文 15-33，不归一化直线方程）
